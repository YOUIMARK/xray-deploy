...
_cf_kill_all() {
    ...
    # 3. 扫全系统杀所有残留 cloudflared 进程
    pids=$(pgrep -x cloudflared 2>/dev/null || ps -o pid,comm 2>/dev/null | grep cloudflared | awk '{print $1}')
    if [ -n "$pids" ]; then
        for pid in $pids; do
    ...
    pids=$(pgrep -x cloudflared 2>/dev/null || ps -o pid,comm 2>/dev/null | grep cloudflared | awk '{print $1}')
    if [ -n "$pids" ]; then
        _warn "cloudflared 仍有残留进程: $pids, 尝试 kill -9"
        for pid in $pids; do
            kill -9 "$pid" 2>/dev/null || true
        done
        sleep 1
...
# 重启 cloudflared service(先杀干净所有, 再重新 start)
_cf_restart() {
    _cf_kill_all
    case "$INIT_SYSTEM" in
...
_cf_switch_token() {
    ...
    # 重启: 先杀干净所有, 验证启动(进程泄漏修复)
    _cf_kill_all
    case "$INIT_SYSTEM" in systemd) systemctl start cloudflared 2>/dev/null ;; *) rc-service cloudflared start 2>/dev/null ;; esac
    sleep 2
